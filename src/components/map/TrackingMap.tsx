...
export default function TrackingMap({ lat, lon, isLost = false }: Props) {
  ...
  useEffect(() => {
    ...
    if (isLost) {
      ...
      pulse.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:60px;height:60px;border-radius:50%;border:2px solid #dc2626;opacity:0;animation:tracking-pulse 2s infinite;pointer-events:none;';
      (el.firstElementChild as HTMLElement).appendChild(pulse);

      if (!document.getElementById('tracking-pulse-style')) {
        const styleEl = document.createElement('style');
        styleEl.id = 'tracking-pulse-style';
        styleEl.textContent = '@keyframes tracking-pulse{0%{transform:translate(-50%,-50%) scale(1);opacity:0.6}100%{transform:translate(-50%,-50%) scale(2);opacity:0}}';
  ...
  // Pan + move marker when position changes — skip untuk paket hilang
  useEffect(() => {
    if (!mapRef.current || !markerRef.current || isLost) return;
